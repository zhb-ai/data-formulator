...
class Client(object):
    ...
    def ping(self, timeout: int = 10):
        ...
        max_tokens=1 and a short timeout.  Raises on any failure."""
    ...
    def get_completion(self, messages, stream=False):
        ...
        if self.endpoint == "openai":
            ...
            try:
                ...
                error_text = str(e)
                # Some text-only models reject multimodal payloads with image_url blocks.
                # Retry once with images stripped to avoid hard-failing the request.
                if self._is_image_deserialize_error(error_text):
                    sanitized_messages = self._strip_images_from_messages(messages)
                    return litellm.completion(
                        model=self.model,
